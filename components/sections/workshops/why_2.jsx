export default function WorkshopsWhy1() {
  return (
    <section id="systemic-why" className="">
      <div className="container mx-auto my-8 lg:my-24">
        <div className="bg-gradient-to-br from-accent via-accent-dark-3 to-accent-dark-7 p-12 m-4 border rounded-2xl shadow-lg text-center text-white">
          <h2 className=" text-accent-light-7 lg:text-6xl">
            Grup Çalışmalarımız
          </h2>
          <div className=" mx-auto bg-accent-light-3 p-4 md:p-12 text-accent-dark-7 rounded-xl my-8">
            <h2 className="">Grubumuza Katılımın Yararları</h2>
            <ul className="grid md:grid-cols-2 mx-auto gap-8 my-8 p-4 [&>li]:border [&>li]:bg-accent-light-7 [&>li]:rounded-xl [&>li]:p-4">
              <li>
                <h4>Gizli Etkileri Ortaya Çıkarın</h4>
                <p>
                  Zorluklarınızı etkileyen sistemik güçler hakkında netlik
                  kazanın, kendinizi bilinçli seçimler yapma ve kalıcı değişim
                  yaratma konusunda özgürleştirin.
                </p>
              </li>
              <li>
                <h4>İyileştirici Dönüşümleri Deneyimleyin</h4>
                <p>
                  Sizi geride tutan sınırlayıcı kalıplardan ve inançlardan
                  kurtulun, kişisel gelişiminizi ve güçlenmenizi teşvik edin.
                </p>
              </li>
              <li>
                <h4>İlişkilerinizi Geliştirin</h4>
                <p>
                  Aileniz, ortaklıklarınız ve diğer önemli bağlantılarınız
                  arasında daha derin bir anlayış ve şefkat geliştirin.
                </p>
              </li>
              <li>
                <h4>Tam Potansiyelinize Adım Atın</h4>
                <p>
                  Amacınızla uyumlu, daha tatmin edici bir yaşam yaratmak için
                  sistemik dizim çalışmalarından elde edilen içgörüleri
                  somutlaştırın.
                </p>
              </li>
              <li>
                <h4>Dönüştürücü Bir Topluluğa Katılın</h4>
                <p>
                  Benzer bir kendini keşfetme yolculuğunda başkalarıyla bağlantı
                  kurun ve birbirinizin gelişimini destekleyin.
                </p>
              </li>

              <li>
                <h4>Grubumuz Nasıl Çalışıyor?</h4>
                <p>
                  Güvenli ve destekleyici bir ortamda aşağıdakileri
                  gerçekleştirmek için çeşitli Sistemik Dizim tekniklerini
                  kullanırız:
                </p>
                <ul>
                  <li>
                    Birbirini destekleyen grup üyeleriyle birlikte gizli ve
                    saygılı bir alanda en kişisel konularınızı güvenle çalışın
                  </li>
                  <li>
                    Grup üyelerinin ortaya çıkardığı kişisel sorunları keşfedin
                  </li>
                </ul>
              </li>
            </ul>
          </div>

          <div className="p-4 m-8 mx-auto border rounded-xl bg-accent-dark-3">
            <h3>Kimler Katılmalı</h3>
            <ul>
              <li>
                Kendilerini ve parçası oldukları sistemleri daha derinlemesine
                anlamaya çalışan bireyler.
              </li>
              <li>
                İlişkilerde, kariyerde, sağlıkta veya kişisel gelişimde
                zorluklarla karşılaşanlar.
              </li>
              <li>
                Sistemik Dizim çalışmasının dönüştürücü gücünden etkilenen
                herkes.
              </li>
            </ul>
            <h5>
              Sistemik Dizimlerin dönüştürücü gücünü keşfedin. Çalışma Grubumuza
              katılın ve potansiyelinizi ortaya çıkarın!
            </h5>
            <p>
              SD Bilgilendirilmiş Onam Formu: Lütfen Grup Çalışmalarına
              Katılmadan önce okuyup onaylayınız.
            </p>

            <h3>
              Kendini Keşfetme ve İyileşmenin Dönüştürücü Yolculuğuna Katılın
            </h3>
            <p>
              Hayatınızı şekillendiren gizli dinamikleri ortaya çıkarmaya hazır
              mısınız? Sistemik Dizim Çalışma Grubumuz, bu birbirine bağlı
              ilişkileri keşfetmek ve derin kişisel gelişimi teşvik etmek için
              güçlü ve dönüştürücü bir alan sunar.
            </p>
          </div>
        </div>
      </div>
    </section>
  );
}
